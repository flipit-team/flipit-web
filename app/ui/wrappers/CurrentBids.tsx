        ...
        <div className='mx-[120px] xs:mx-0 my-6 xs:my-0'>
            ...
            {bids?.length ? (
                <div className='shadow-lg xs:shadow-transparent flex flex-col gap-6 p-8 xs:p-4'>
                    {bids?.map((bid, i) => {
                        const imageUrl =
                            bid?.auctionItem?.imageUrls && bid.auctionItem.imageUrls.length > 0
                                ? bid.auctionItem.imageUrls[0]
                                : '/placeholder-product.svg';

                        // Check if this item belongs to the current user
                        const isOwnItem =
                            user?.userId && bid?.auctionItem?.seller?.id?.toString() === user.userId;
                        const href = isOwnItem ? `/manage-item/${bid?.auctionItem?.id}` : `/${bid?.auctionItem?.id}`;

                        return (
                            <Link
                                key={i}
                                href={href}
                                className='flex items-center border border-border_gray rounded-lg w-[672px] xs:w-full p-3 hover:border-primary/30 transition-colors cursor-pointer'
                            >
                                <Image
                                    src={imageUrl}
                                    height={64}
                                ...
                                <div>
                                    <div className='flex items-center'>
                                        <p className='text-primary typo-body_ls xs:typo-body_ms mr-2'>
